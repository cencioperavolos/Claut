...
const isExpressionOwnerOrAdmin = require('../middleware').isExpressionOwnerOrAdmin
...
router.get('/:id', async (req, res, next) => {
  try {
    const expression = await Expression.findById(req.params.id).populate('words').exec()
...
router.put('/:id', isExpressionOwnerOrAdmin, async (req, res, next) => {
  ...
  try {
    const exp = await Expression.findByIdAndUpdate(req.params.id, req.body.expression, {
      new: true,
      runValidators: true
    })
    if (!exp) {
...
router.delete('/:id', isExpressionOwnerOrAdmin, async (req, res, next) => {
  try {
    ...
    req.flash('success', 'Espressione "' + deleted.clautano + '" eliminata definitivamente.')
    res.redirect('/')
  } catch (e) {
    next(e)
  }
